...
tuple<int, int, bool, Particle (*)(int, int)> click;
...
void drawParticle(int x, int y, Color3 color) {
    if (x < 0 || x > 71 || y < 0 || y > 71) {return;}
    int x1 = x * PARTICLE_SIZE;
    int y1 = y * PARTICLE_SIZE;
    for (int x2 = x1; x2 < x1 + PARTICLE_SIZE; x2++) {
        for (int y2 = y1; y2 < y1 + PARTICLE_SIZE; y2++) {
...
void idle() {
    ...
    if (get<2>(click)) {
        int x = get<0>(click), y = get<1>(click);
...
void onMouse(int button, int state, int x, int y) {
    ...
    click = tuple<int, int, bool, Particle (*)(int, int)>(x1, y1, state == GLUT_DOWN, p);
...
void onMotion(int x, int y) {
    if (get<2>(click)) {
        ...
        y1 = y1 / PARTICLE_AMOUNT - 0.5;
        y1 = round((-y1 + 0.5) * PARTICLE_AMOUNT) - 1;
        click = tuple<int, int, bool, Particle (*)(int, int)>(x1, y1, true, get<3>(click));
